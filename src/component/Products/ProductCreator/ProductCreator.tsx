...
import { Warehouse, getAdminGroups,  getWarehouses, createProduct,  ProductCreation} from '../ProductsApi';
import { currencyMap } from '../../../constants/MapConstants';
import { useKeycloak } from '@react-keycloak/web';
function ProductCreator({setShowProductCreator}:{setShowProductCreator :  (show :boolean) => void}){
    ...
    const [productWarehouses, setProductWarehouses] = useState<Map<number, {warehouseId: string, row:  number, shelfNumber: number, tierNumber:  number, amount: number}>>(new Map());
    ...
    useEffect(() => {
      ...
      const updateProductWarehouses = (elementId: number, detailType: string, value: string) => {
        setProductWarehouses(prevDetails => {
          ...
          const detail: { 
            ...
            tierNumber: number; 
            amount: number; 
          } = updatedDetails.get(elementId) || { warehouseId: '', row: 0, shelfNumber: 0, tierNumber: 0, amount: 0 };
    ...
    const sendCreationProductRequest =async () =>{
      ...
      const warehouses = Array.from(productWarehouses.values()).filter(wh => {
        return wh.warehouseId || wh.row || wh.shelfNumber || wh.tierNumber || wh.amount;
      });
    
      const fetchedProduct = await createProduct(keycloak.token, {
        ...productData,
       groupId: groupId,
          ...
          <CreationInputContainer>
            <ProductGrid>
              ...
              <Flex width='100%' align='center' justify='space-between' key={elementId}>
                ...
                <Flex gap="1em">no. row
                  <ShortCreationInput onChange={(e) => updateProductWarehouses(elementId, 'row', e.target.value)}></ShortCreationInput>
                </Flex>
                <Flex >no. shelf
                  <ShortCreationInput onChange={(e) => updateProductWarehouses(elementId, 'shelf', e.target.value)}></ShortCreationInput>
                </Flex>
                <Flex>no. tier
                  <ShortCreationInput onChange={(e) => updateProductWarehouses(elementId, 'tier', e.target.value)}></ShortCreationInput>
                </Flex>
                <Flex >amount 
